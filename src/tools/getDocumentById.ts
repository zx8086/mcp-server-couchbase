...
export default (server: McpServer, bucket: Bucket) => {
    server.tool(
        ...
        async ({ arguments: args }) => {
            docLogger.info("Handler received arguments:", { args, type: typeof args });

            const scope_name = args?.scope_name || args?.scope || "";
            const collection_name = args?.collection_name || args?.collection || "";
            const document_id = args?.document_id || args?.id || "";

            docLogger.info("Final values:", { scope_name, collection_name, document_id });

            try {
                docLogger.info(`Getting document by ID`, { scope_name, collection_name, document_id });
