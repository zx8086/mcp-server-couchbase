...
export function registerDocumentResource(server: McpServer, bucket: Bucket): void {
  server.resource(
    "document",
    new ResourceTemplate("document://{scope}/{collection}/{id}", { list: undefined }),
    async (uri, { scope, collection, id }) => {
      try {
        resourceLogger.info('Fetching document resource', { scope, collection, id });
        ...
        try {
          ...
          if (docError instanceof Error && docError.name === 'DocumentNotFoundError') {
            return {
              contents: [{
                uri: uri.href,
                type: "text/plain",
          ...
          } else if (docError instanceof Error && docError.message.includes('scope not found')) {
            return {
              contents: [{
                uri: uri.href,
                type: "text/plain",
          ...
          } else if (docError instanceof Error && docError.message.includes('collection not found')) {
            return {
              contents: [{
                uri: uri.href,
                type: "text/plain",
